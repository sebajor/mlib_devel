...
for ctr = 1 : numel(xps_blks),
    ...
    if any(strfind(xps_blks{ctr}, ' ')) && ~any(regexpi(xps_blks{ctr}, 'XSG core config$')),
        error('casper:space_in_xps_block', 'Block names may not have spaces - %s', xps_blks{ctr});
    end 
end
for ctr = 1 : numel(casper_blks),
    if any(strfind(casper_blks{ctr}, ' '))
        error('casper:space_in_casper_block', 'Block names may not have spaces - %s', casper_blks{ctr});
    end 
end
