function filt_init(blk,varargin)

clog('entering filt_init', 'trace');
...
for i=1:num_fir_col,
    ...
    if i == 1,
        for j=1:n_inputs,
            add_line(blk, ['regis',num2str(j),'/1'], [blk_name,'/',num2str(2*j-1)]);
            if j <= n_inputs/2,
                add_line(blk, ['regis',num2str(j+n_inputs/2),'/1'], [blk_name,'/',num2str(2*j)]);
            else
                add_line(blk, ['in',num2str(j-n_inputs/2),'/1'], [blk_name,'/',num2str(2*j)]);
            end
        end    
    else
        for j=1:n_inputs,
...
reuse_block(blk, 'dout2', 'built-in/outport', ...
    'Position', [200*num_fir_col+680 400+30 200*num_fir_col+710 415+30], 'Port', '3');
...
add_line(blk, 'convert2/1', 'dout2/1');
