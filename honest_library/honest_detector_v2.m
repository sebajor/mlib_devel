...
nbits = log2(n_channels);                   % Number of bits of counter
...
if n_channels <= 32
    ...
    mux_input = [{sel},mux_signals];  % Add 'sel' signal to the first element of mux inputs
...
if (n_channels <= 1024) && (n_channels > 32)
    ...
    slice_common = xBlock('Slice',struct('nbits',5,'mode','Lower Bit Location + Width'),{sel},{common_mux});               % Select bits of 'Count' common muxes
    slice_stage = xBlock('Slice',struct('nbits',bits_selector,'mode','Upper Bit Location + Width'),{sel},{stage_mux});     % Select bits of 'Count' stage mux
